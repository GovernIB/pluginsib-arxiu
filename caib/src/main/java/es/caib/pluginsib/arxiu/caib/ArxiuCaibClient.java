...
public class ArxiuCaibClient {
	...
	public <T, U, V> V generarEnviarPeticio(
			...
			Class<V> respostaType)
			     throws IllegalAccessException, IllegalArgumentException, InvocationTargetException, InstantiationException, UniformInterfaceException, ClientHandlerException, IOException, NoSuchMethodException, SecurityException {
		...
		T peticio = peticioType.getConstructor().newInstance();
	...
	private JerseyResponse enviarPeticioArxiu(
		...
		if (this.debug) {
			boolean logRequestBody = this.debug || logger.isTraceEnabled();
			String msg = "Enviant petició HTTP a l'arxiu (" +
					"url=" + urlAmbMetode + ", " +
		...
		if (this.debug) {
			boolean logResponseBody = this.debug || logger.isTraceEnabled();
			String msg = "Rebuda resposta HTTP de l'arxiu ("

...
public class ArxiuConversioHelper {
	...
	private static Date parseDateIso8601(String date) throws ArxiuException {
		...
		try {
			...
			c.setTime(ISO8601ToDate(date));
			return c.getTime();
		} catch (IllegalArgumentException ex) {
			throw new ArxiuException("No s'ha pogut parsejar el valor de la data (valor=" + date + "): " + ex.getMessage());
		} catch (ParseException e) {
		    throw new ArxiuException("No s'ha pogut parsejar el valor de la data (valor=" + date + "): " + e.getMessage());
        }
	}
